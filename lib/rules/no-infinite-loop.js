...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      unconditionalSelfTransition:
        'Eventless transition cannot target its own state node unconditionally. This will cause an infinite loop error.',
      ...
      noTargetAndGuardNotCheckingContext:
        'An eventless transition with no target and with a guard not using the machine context is either useless (never taken) or will cause an infinite loop.',
      ...
      'Property[key.name="always"] > ArrayExpression > ObjectExpression': function (
        ...
        if (!hasProperty('cond', node) && targetsItself) {
          if (isFirstArrayItem(node)) {
            context.report({
              node,
              messageId: 'unconditionalSelfTransition',
            })
            return
          }
          context.report({
        ...
        if (
          ...
          // or an infinite loop.
          if (guardIsFunctionAndNotCheckingContext(node)) {
            context.report({
              node,
