...
module.exports = {
  ...
  create: function (context) {
    ...
    return {
      'CallExpression[callee.name=/^createMachine$|^Machine$/] Property[key.name="states"] > ObjectExpression > Property': function (
        node
      ) {
        ...
        if (name !== fixedName) {
          ...
          context.report({
            ...
            data: { fixedName, name },
            fix(fixer) {
              return fixer.replaceText(node.key, `${quote}${fixedName}${quote}`)
            },
          })
        }
      },
