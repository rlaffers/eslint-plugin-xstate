...
const { isFirstArrayItem, propertyHasName, hasProperty } = require('../utils/commonMatchers')
...
function isUnconditionalTransitionToItself(transition) {
  ...
  const targetStateNodeNameOrID = transition.properties.find(propertyHasName('target')).value.value
...
module.exports = {
  ...
  create: function (context) {
    return {
      ...
      'Property[key.name="always"] > ArrayExpression > ObjectExpression': function (node) {
        if (!isInsideMachineDeclaration(node)) {
          return
        }
        // always: [{}]
